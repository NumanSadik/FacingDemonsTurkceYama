...
-------------------------------------------------------------------------------
function qphue_to_rgb(p,q,t) -- 0.4 , 1
    if (t < 0) then 
	while t<0 do 
		t = t + 1 
	end 
...
function forme2(p,q,t) -- 0.4 , 1
    ...
    outgo=nil
	if (t<1/6) then
		outgo=0.4*math.sin(math.pi*t/(1/3))
		elseif t<2/6 then
		outgo=0.4+0.2*math.sin(math.pi*(t-1/6)/(1/3))
		elseif t<4/6 then
		outgo=0.6+0.4*math.sin(math.pi*(t-2/6)/(1/3))
		elseif t<5/6 then
		outgo=0.6-0.2*math.sin(math.pi*(t-4/6)/(1/3))
		else
		outgo=0.4-0.4*math.sin(math.pi*(t-5/6)/(1/3))
	end
	return outgo
end
...
function forme1(hue, saturation, lightness)
	local q = nil
	
	if lightness < 0.5 then
	    q = lightness * (1 + saturation)
	else
		q = lightness + saturation - lightness * saturation--q=1
	end
	
	local p = 2 * lightness - q--p=0.4

	return{forme2(p, q, (hue / (2 * math.pi)) + 1 / 3),	--  (0.4 , 1 , num)
		forme2(p, q, (hue / (2 * math.pi))), 		--
		forme2(p, q, (hue / (2 * math.pi)) - 1 / 3)}	--
end
...
function hsv_to_rgb(hue, saturation, lightness)
	...
	return{qphue_to_rgb(p, q, (hue / (2 * math.pi)) + 1 / 3),	--  (0.4 , 1 , num)
...
function color_multiply(color, multiplier)
    return {math.min(color[1] * multiplier, 1.0), math.min(color[2] * multiplier, 1.0), math.min(color[3] * multiplier, 1.0),1.0}
end
