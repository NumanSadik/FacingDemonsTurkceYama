...
for n=1,#kniferainleft do
  ...
  if timer==limit then
	...
	knive1.sprite.alpha=0
	knive2.sprite.alpha=0
	knive3.sprite.alpha=0
	knive4.sprite.alpha=0
...
for i=1,#targetattack do
  ...
  if not rew[i]  then
    if (Player.x-bullet.x)>200 and (Player.x-bullet.x)<220 then
      right[i]=1
      elseif (Player.x-bullet.x)>-220 and (Player.x-bullet.x)<-200 then
      right[i]=-1 
      else right[i]=0
    end
    if (Player.y-bullet.y)>200 and (Player.y-bullet.y)<220 then
      up[i]=1
      elseif (Player.y-bullet.y)>-220 and (Player.y-bullet.y)<-200 then
      up[i]=-1 
      else up[i]=0
    end
    rew[i]=43
    elseif rew[i]>12 then
      rew[i]=rew[i]-1
	bullet.Move(right[i],up[i])
	local t=42-rew[i]
	bullet.sprite.alpha=math.sin(t*math.pi/60)
